"""为测试租户填充全部模块的测试数据（参数化：tenant-qa / tenant-default）。
...
用法：
    ...
    （默认同时 seed tenant-qa 与 tenant-default；亦可分别调用 seed_test_tenant()/seed_default_tenant()）
"""
...
# 租户配置：由 configure() 设置；默认保持 tenant-qa 行为（向后兼容）。
SEED_TENANT = "tenant-qa"
TENANT_NAME = "QA 测试组织"
TENANT_SLUG = "qa-test"
_ID_PREFIX = "q"
_PRESERVE_TENANT = False


def configure(tenant_id: str, name: str, slug: str, prefix: str, preserve_tenant: bool = False) -> None:
    """设置 seed 目标租户与 ID 前缀；不同租户用不同前缀避免主键冲突。"""
    global SEED_TENANT, TENANT_NAME, TENANT_SLUG, _ID_PREFIX, _PRESERVE_TENANT
    SEED_TENANT = tenant_id
    TENANT_NAME = name
    TENANT_SLUG = slug
    _ID_PREFIX = prefix
    _PRESERVE_TENANT = preserve_tenant


def _id(suffix: str) -> str:
    return f"{_ID_PREFIX}{suffix}"
...
def _clear(db: Session) -> None:
    ...
    if not _PRESERVE_TENANT:
        # 独立测试租户（如 tenant-qa）可整体删重建
    ...
    # preserve_tenant=True（如 tenant-default）：保留租户行与成员关系，仅清理业务数据
    db.commit()


# ============ 开发者画像（详情页 JSON）============
def _dev_details() -> dict:
    return {
        _id("d1"): dict(
            ...
            modules=[{"module": "auth-core", "commits": 132, "ownership": 70, "complexity": 66, "projectId": _id("p1"), "projectName": "用户中心"}, {"module": "gateway-config", "commits": 58, "ownership": 44, "complexity": 30, "projectId": _id("p1"), "projectName": "用户中心"}],
        ...
        _id("d2"): dict(
            ...
            modules=[{"module": "order-core", "commits": 178, "ownership": 66, "complexity": 70, "projectId": _id("p2"), "projectName": "订单系统"}, {"module": "inventory", "commits": 92, "ownership": 58, "complexity": 48, "projectId": _id("p2"), "projectName": "订单系统"}],
        ...
        _id("d3"): dict(
            ...
            modules=[{"module": "design-system", "commits": 126, "ownership": 76, "complexity": 50, "projectId": _id("p3"), "projectName": "内容引擎"}, {"module": "content-renderer", "commits": 98, "ownership": 66, "complexity": 62, "projectId": _id("p3"), "projectName": "内容引擎"}],
        ...
        _id("d4"): dict(
            ...
            modules=[{"module": "content-api", "commits": 88, "ownership": 55, "complexity": 46, "projectId": _id("p3"), "projectName": "内容引擎"}],
        ...
        _id("d5"): dict(
            ...
            modules=[{"module": "deploy-platform", "commits": 96, "ownership": 62, "complexity": 44, "projectId": _id("p4"), "projectName": "交付平台"}],
        ...
        _id("d6"): dict(
            ...
            modules=[{"module": "search-index", "commits": 74, "ownership": 58, "complexity": 50, "projectId": _id("p5"), "projectName": "搜索中台"}],
...
def _seed_org_and_teams(db: Session) -> None:
    db.add_all([
        models.TeamSpace(id=_id("a-root"), name="测试研发中心", description="QA 测试组织的研发总部，负责演示全模块数据", parent_id=None, status="active", created_at="", updated_at="", member_ids=[], project_ids=[]),
        models.TeamSpace(id=_id("a-test-team"), name="测试团队", parent_id=_id("a-root"), description="本组织的主演示团队：测试团队", owner_id=_id("d1"), owner_name="陈思", status="active", created_at="2026-01-10", updated_at="今天 10:32", member_ids=[_id("d1"), _id("d2"), _id("d3"), _id("d4"), _id("d5"), _id("d6")], project_ids=[_id("p1"), _id("p2"), _id("p3"), _id("p4"), _id("p5"), _id("p6")]),
        models.TeamSpace(id=_id("a-backend"), name="后端小组", parent_id=_id("a-test-team"), description="负责账户、订单、风控等后端核心链路", owner_id=_id("d1"), owner_name="陈思", status="active", created_at="2026-01-12", updated_at="今天 09:12", member_ids=[_id("d1"), _id("d2")], project_ids=[_id("p1"), _id("p2")]),
        models.TeamSpace(id=_id("a-frontend"), name="前端小组", parent_id=_id("a-test-team"), description="负责内容体验、设计系统与用户端工程", owner_id=_id("d3"), owner_name="王琳", status="active", created_at="2026-01-14", updated_at="昨天", member_ids=[_id("d3"), _id("d4")], project_ids=[_id("p3")]),
        models.TeamSpace(id=_id("a-infra"), name="基础设施小组", parent_id=_id("a-test-team"), description="负责交付平台、稳定性和基础设施", owner_id=_id("d5"), owner_name="刘洋", status="active", created_at="2026-02-02", updated_at="3 小时前", member_ids=[_id("d5")], project_ids=[_id("p4"), _id("p5")]),
    ])
    db.commit()

    db.add_all([
        models.Team(id=_id("a-t1"), name="测试团队", members=6, avg_score=86, bus_factor=3, risk_count=1, capability={"code_quality": 87, "architecture": 88, "stability": 84, "efficiency": 82, "collaboration": 85, "security_aware": 78, "test_coverage": 76}),
        models.Team(id=_id("a-t2"), name="后端小组", members=2, avg_score=88, bus_factor=2, risk_count=0, capability={"code_quality": 88, "architecture": 90, "stability": 85, "efficiency": 83, "collaboration": 87, "security_aware": 81, "test_coverage": 82}),
        models.Team(id=_id("a-t3"), name="前端小组", members=2, avg_score=85, bus_factor=1, risk_count=1, capability={"code_quality": 86, "architecture": 82, "stability": 80, "efficiency": 84, "collaboration": 86, "security_aware": 76, "test_coverage": 76}),
        models.Team(id=_id("a-t4"), name="基础设施小组", members=2, avg_score=80, bus_factor=2, risk_count=1, capability={"code_quality": 80, "architecture": 84, "stability": 83, "efficiency": 76, "collaboration": 78, "security_aware": 82, "test_coverage": 70}),
    ])
    db.commit()
...
def _seed_developers(db: Session) -> None:
    devs = [
        dict(id=_id("d1"), name="陈思", username="chensi", role="架构师", role_type="backend", team="后端小组", team_id=_id("a-backend"), level="E3", overall=88, commits=402, reviews=151, langs=["Go", "Python"], tags=["核心模块贡献者", "偏后端架构"]),
        dict(id=_id("d2"), name="林涛", username="lintao", role="技术专家", role_type="backend", team="后端小组", team_id=_id("a-backend"), level="D2", overall=90, commits=508, reviews=190, langs=["Java", "Kotlin"], tags=["核心模块贡献者", "架构能力突出"]),
        dict(id=_id("d3"), name="王琳", username="wanglin", role="高级工程师", role_type="frontend", team="前端小组", team_id=_id("a-frontend"), level="E3", overall=87, commits=376, reviews=128, langs=["TypeScript", "React"], tags=["核心模块贡献者", "前端专家"]),
        dict(id=_id("d4"), name="周杰", username="zhoujie", role="工程师", role_type="frontend", team="前端小组", team_id=_id("a-frontend"), level="F3", overall=76, commits=258, reviews=72, langs=["TypeScript", "Vue"], tags=["协作能力突出"]),
        dict(id=_id("d5"), name="刘洋", username="liuyang", role="高级工程师", role_type="devops", team="基础设施小组", team_id=_id("a-infra"), level="E1", overall=79, commits=302, reviews=88, langs=["Go"], tags=["稳定性强", "运维能力"]),
        dict(id=_id("d6"), name="赵磊", username="zhaolei", role="技术专家", role_type="algorithm", team="基础设施小组", team_id=_id("a-infra"), level="D3", overall=90, commits=588, reviews=232, langs=["Python", "Rust"], tags=["全栈能力", "成长速度快"]),
    ]
    details = _dev_details()
    for d in devs:
...
def _seed_projects(db: Session) -> None:
    projects = [
        dict(id=_id("p1"), name="用户中心", group="后端小组", team_id=_id("a-backend"), language="Go", score=88, quality=90, security=85, debt=22, status="completed", commits=1847, contributors=3, last_analyzed="2小时前"),
        dict(id=_id("p2"), name="订单系统", group="后端小组", team_id=_id("a-backend"), language="Java", score=90, quality=92, security=88, debt=18, status="completed", commits=3214, contributors=2, last_analyzed="1小时前"),
        dict(id=_id("p3"), name="内容引擎", group="前端小组", team_id=_id("a-frontend"), language="TypeScript", score=92, quality=94, security=90, debt=15, status="completed", commits=2891, contributors=3, last_analyzed="4小时前"),
        dict(id=_id("p4"), name="交付平台", group="基础设施小组", team_id=_id("a-infra"), language="Go", score=79, quality=81, security=82, debt=25, status="completed", commits=2034, contributors=2, last_analyzed="8小时前"),
        dict(id=_id("p5"), name="搜索中台", group="基础设施小组", team_id=_id("a-infra"), language="Python", score=81, quality=83, security=79, debt=28, status="completed", commits=1567, contributors=2, last_analyzed="6小时前"),
        dict(id=_id("p6"), name="消息推送", group="测试团队", team_id=_id("a-test-team"), language="Go", score=74, quality=76, security=72, debt=32, status="pending", commits=892, contributors=1, last_analyzed="待分析"),
    ]
    for p in projects:
...
def _seed_project_details(db: Session) -> None:
    details = {
        _id("p1"): dict(
            ...
            insights=[
                dict(id=_id("ins-1"), title="JWT 校验未限制签名算法", module="auth-core", type="security", category="security", severity="high", level="warning", risk_score=91, confidence=0.94, status="open", file_path="services/auth/token.ts", symbol="verifyToken", start_line=42, end_line=58, source="SAST + AI Review", first_seen_at="2026-07-18", last_seen_at="2026-07-26", assignee="陈思", evidence="3 处 JWT 校验未检查签名算法", code_excerpt="jwt.verify(token, publicKey, options)", impact="潜在算法混淆攻击，可能绕过身份校验。", action="强制 RS256 算法白名单，并增加拒绝非白名单算法的测试。", verification="安全规则重扫通过，新增算法拒绝单测。"),
                dict(id=_id("ins-2"), title="Redis 连接未复用", module="session-mgr", type="performance", category="reliability", severity="high", level="warning", risk_score=82, confidence=0.91, status="in_progress", file_path="services/session/redis-client.ts", symbol="getSession", start_line=18, end_line=35, source="Performance Rule + AI Review", first_seen_at="2026-07-03", last_seen_at="2026-07-26", assignee="周杰", evidence="每次请求新建 Redis 客户端，缺少连接池", code_excerpt="const client = createClient({ url: redisUrl })", impact="P99 延迟偏高（180ms）", action="引入连接池、超时和故障转移策略。", verification="压测 P99 低于 80ms。"),
                dict(id=_id("ins-3"), title="生产环境允许 Debug 日志", module="gateway-config", type="configuration", category="configuration", severity="medium", level="warning", risk_score=72, confidence=0.79, status="open", file_path="deploy/values-prod.yaml", symbol="", start_line=24, end_line=24, source="Config Scanner", first_seen_at="2026-07-26", last_seen_at="2026-07-26", assignee="刘洋", evidence="production values 中 LOG_LEVEL 设置为 debug", code_excerpt="LOG_LEVEL: debug", impact="可能记录敏感请求上下文", action="生产环境改为 info。", verification="部署清单重扫无生产 debug 配置。"),
            ],
            module_risks=[
                dict(id=_id("mod-1"), name="auth-core", path="services/auth", score=84, severity="high", critical_count=0, issue_count=2, complexity=68, debt_load=18, owner="陈思", backup_owner="刘洋", ownership=72, last_changed="2 小时前", categories=[{"category": "security", "count": 1}, {"category": "logic", "count": 1}]),
                dict(id=_id("mod-2"), name="session-mgr", path="services/session", score=78, severity="high", critical_count=0, issue_count=2, complexity=57, debt_load=12, owner="周杰", ownership=48, last_changed="昨天", categories=[{"category": "performance", "count": 1}, {"category": "reliability", "count": 1}]),
                dict(id=_id("mod-3"), name="user-core", path="modules/user", score=65, severity="medium", critical_count=0, issue_count=3, complexity=83, debt_load=10, owner="刘洋", backup_owner="陈思", ownership=65, last_changed="3 天前", categories=[{"category": "complexity", "count": 2}, {"category": "maintainability", "count": 1}]),
            ],
            fixes=[
                dict(id=_id("fix-1"), insight_id=_id("ins-1"), module="auth-core", title="限制 JWT 签名算法", severity="high", priority="P0", debt=18, effort="3 人日", impact="消除身份校验绕过风险", expected_gain=7, status="open", assignee="陈思", due_date="8 月 4 日"),
                dict(id=_id("fix-2"), insight_id=_id("ins-2"), module="session-mgr", title="引入 Redis 连接池", severity="high", priority="P1", debt=12, effort="2 人日", impact="P99 延迟预计降至 80ms", expected_gain=4, status="in_progress", assignee="周杰", due_date="8 月 9 日"),
            ],
        ),
        _id("p2"): dict(
            project=dict(dimensions=[
            ...
            insights=[
                dict(id=_id("ins-4"), title="金额计算使用浮点数", module="payment-gw", type="security", category="logic", severity="high", level="warning", risk_score=79, confidence=0.9, status="open", file_path="payment/MoneyCalculator.java", symbol="calculateFee", start_line=33, end_line=47, source="Logic Rule + AI Review", first_seen_at="2026-07-20", last_seen_at="2026-07-26", assignee="林涛", evidence="金额计算未使用 Decimal/BigDecimal", code_excerpt="double fee = amount * rate;", impact="浮点精度误差可能导致订单金额不一致", action="统一使用 BigDecimal", verification="金额精度规则通过"),
                dict(id=_id("ins-5"), title="订单状态机是可复用最佳实践", module="order-core", type="quality", category="quality", severity="info", level="info", risk_score=12, confidence=0.96, status="resolved", file_path="order/OrderStateMachine.kt", symbol="OrderStateMachine", start_line=1, end_line=208, source="AI Review", first_seen_at="2026-07-26", last_seen_at="2026-07-26", assignee="林涛", evidence="状态转换完整，测试覆盖充分", impact="维护成本低", action="沉淀为团队订单领域建模范式", verification="架构评审通过"),
            ],
            module_risks=[
                dict(id=_id("mod-4"), name="payment-gw", path="payment", score=73, severity="high", critical_count=0, issue_count=1, complexity=45, debt_load=14, owner="林涛", backup_owner="陈思", ownership=55, last_changed="5 小时前", categories=[{"category": "logic", "count": 1}]),
                dict(id=_id("mod-5"), name="order-core", path="order", score=24, severity="low", critical_count=0, issue_count=0, complexity=38, debt_load=3, owner="林涛", backup_owner="陈思", ownership=68, last_changed="昨天", categories=[{"category": "quality", "count": 0}]),
            ],
            fixes=[
                dict(id=_id("fix-3"), insight_id=_id("ins-4"), module="payment-gw", title="替换为 BigDecimal 金额模型", severity="high", priority="P1", debt=14, effort="2 人日", impact="消除金额精度风险", expected_gain=4, status="open", assignee="林涛", due_date="8 月 6 日"),
            ],
        ),
        _id("p3"): dict(
            project=dict(dimensions=[
            ...
            insights=[
                dict(id=_id("ins-6"), title="内联样式硬编码颜色", module="design-system", type="maintainability", category="maintainability", severity="low", level="info", risk_score=34, confidence=0.86, status="open", file_path="components/hero/banner.tsx", symbol="Banner", start_line=12, end_line=40, source="Frontend Rule + AI Review", first_seen_at="2026-07-25", last_seen_at="2026-07-26", assignee="周杰", evidence="2 处使用 #1890ff 硬编码颜色", code_excerpt="style={{ color: '#1890ff' }}", impact="视觉不一致，后续换肤成本高", action="替换为设计 token", verification="token 重扫通过"),
            ],
            module_risks=[
                dict(id=_id("mod-6"), name="design-system", path="components", score=58, severity="medium", critical_count=0, issue_count=1, complexity=52, debt_load=8, owner="王琳", backup_owner="周杰", ownership=76, last_changed="今天", categories=[{"category": "maintainability", "count": 1}]),
                dict(id=_id("mod-7"), name="content-renderer", path="renderer", score=44, severity="medium", critical_count=0, issue_count=0, complexity=62, debt_load=5, owner="王琳", backup_owner="周杰", ownership=66, last_changed="昨天", categories=[{"category": "complexity", "count": 0}]),
            ],
            fixes=[
                dict(id=_id("fix-4"), insight_id=_id("ins-6"), module="design-system", title="内联样式替换为设计 token", severity="low", priority="P3", debt=8, effort="1 人日", impact="提升视觉一致性与换肤能力", expected_gain=2, status="open", assignee="周杰", due_date="8 月 20 日"),
            ],
        ),
        _id("p4"): dict(
            project=dict(dimensions=[
              ...
              graph_edges=[]),
            insights=[
                dict(id=_id("ins-7"), title="发布流水线缺少门禁", module="deploy-platform", type="reliability", category="reliability", severity="medium", level="warning", risk_score=66, confidence=0.82, status="acknowledged", file_path="deploy/pipeline.yaml", symbol="", start_line=41, end_line=41, source="Delivery Rule + AI Review", first_seen_at="2026-07-15", last_seen_at="2026-07-26", assignee="刘洋", evidence="流水线未配置测试覆盖门禁", code_excerpt="steps: [build, push]", impact="低质量代码可能直接上线", action="增加覆盖率门禁与人工审批", verification="流水线门禁已配置"),
            ],
            module_risks=[
                dict(id=_id("mod-8"), name="deploy-platform", path="deploy", score=62, severity="medium", critical_count=0, issue_count=1, complexity=44, debt_load=10, owner="刘洋", ownership=62, last_changed="2 天前", categories=[{"category": "reliability", "count": 1}]),
            ],
            fixes=[
                dict(id=_id("fix-5"), insight_id=_id("ins-7"), module="deploy-platform", title="流水线增加覆盖率门禁", severity="medium", priority="P2", debt=10, effort="2 人日", impact="防止低质量代码上线", expected_gain=3, status="acknowledged", assignee="刘洋", due_date="8 月 14 日"),
            ],
        ),
        _id("p5"): dict(
            project=dict(dimensions=[
            ...
            insights=[],
            module_risks=[
                dict(id=_id("mod-9"), name="search-index", path="indexer", score=71, severity="high", critical_count=0, issue_count=1, complexity=66, debt_load=12, owner="赵磊", backup_owner="刘洋", ownership=63, last_changed="今天", categories=[{"category": "complexity", "count": 1}]),
            ],
            fixes=[],
        ),
        _id("p6"): dict(
...
def _seed_repos_identity_gaps(db: Session) -> None:
    db.add_all([
        models.Repository(id=_id("r1"), name="用户中心", path="/data/repos/user-center", source_type="remote", provider="gitlab", remote_url="https://gitlab.example.com/qa/user-center.git", branch="main", team_id=_id("a-backend"), project_id=_id("p1"), status="synced", last_sync="2分钟前", commits=1847, contributors=3),
        models.Repository(id=_id("r2"), name="订单系统", path="/data/repos/order-sys", source_type="remote", provider="github", remote_url="https://github.com/acme/order-system.git", branch="main", team_id=_id("a-backend"), project_id=_id("p2"), status="synced", last_sync="1小时前", commits=3214, contributors=2),
        models.Repository(id=_id("r3"), name="内容引擎", path="/data/repos/content-engine", source_type="remote", provider="github", remote_url="https://github.com/acme/content-engine.git", branch="main", team_id=_id("a-frontend"), project_id=_id("p3"), status="synced", last_sync="4小时前", commits=2891, contributors=3),
        models.Repository(id=_id("r4"), name="交付平台", path="/data/repos/delivery-platform", source_type="local", branch="main", team_id=_id("a-infra"), project_id=_id("p4"), status="synced", last_sync="8小时前", commits=2034, contributors=2),
        models.Repository(id=_id("r5"), name="搜索中台", path="/data/repos/search", source_type="local", branch="main", team_id=_id("a-infra"), project_id=_id("p5"), status="failed", last_sync="2天前", commits=1567, contributors=2),
        models.Repository(id=_id("r6"), name="消息推送", path="/data/repos/push", source_type="remote", provider="github", remote_url="https://github.com/acme/push.git", branch="main", team_id=_id("a-test-team"), project_id=_id("p6"), status="syncing", last_sync="同步中", commits=892, contributors=1),
    ])
    db.add_all([
        models.IdentityMatch(id=_id("im1"), git_name="chensi42", git_email="chensi@qa.com", person_name="陈思", department="后端小组", confidence=1.0, method="email"),
        models.IdentityMatch(id=_id("im2"), git_name="Lin Tao", git_email="lintao@github.com", person_name="林涛", department="后端小组", confidence=0.9, method="employee_id"),
        models.IdentityMatch(id=_id("im3"), git_name="wanglin_dev", git_email="wl123@qq.com", person_name="王琳", department="前端小组", confidence=0.75, method="pinyin"),
        models.IdentityMatch(id=_id("im4"), git_name="zhaolei88", git_email="zl88@163.com", person_name="赵磊", department="基础设施小组", confidence=0.85, method="fuzzy"),
    ])
    db.add_all([
        models.CapabilityGap(id=_id("cg1"), capability="安全意识", current=74, target=85, owner="测试团队", action="安全培训 + 代码审查配对"),
        models.CapabilityGap(id=_id("cg2"), capability="测试覆盖", current=76, target=85, owner="前端小组", action="引入覆盖率门禁 + 测试用例补齐"),
        models.CapabilityGap(id=_id("cg3"), capability="架构能力", current=80, target=88, owner="基础设施小组", action="架构评审会 + 跨组技术分享"),
        models.CapabilityGap(id=_id("cg4"), capability="交付效率", current=82, target=88, owner="后端小组", action="流程优化 + 工具链建设"),
    ])
    db.commit()
...
def _seed_skills(db: Session) -> None:
    ...
    db.add_all([
        models.SkillSource(
            id=_id("src-1"), name="测试-Java编码规范", doc_type="markdown",
            source_lang="java", description="Java 后端服务编码规范测试示例",
            status="extracted", created_at=now, updated_at=now,
            content="# Java 编码规范\n\n- SQL 必须使用参数化查询。\n- 金额计算必须使用 BigDecimal。",
        ),
        models.SkillSource(
            id=_id("src-2"), name="测试-前端编码规范", doc_type="markdown",
    ...
    db.commit()
    skills = [
        dict(id=_id("sk-1"), source_id=_id("src-1"), name="SQL 禁止字符串拼接", description="SQL 必须参数化查询", category="security", severity="high",
        ...
        dict(id=_id("sk-2"), source_id=_id("src-1"), name="金额使用 BigDecimal", description="金额计算禁止浮点", category="security", severity="high",
        ...
        dict(id=_id("sk-3"), source_id=_id("src-2"), name="禁止 console.log 提交", description="调试代码不得入主分支", category="quality", severity="medium",
        ...
        dict(id=_id("sk-4"), source_id=_id("src-2"), name="禁止内联样式硬编码", description="颜色间距走设计 token", category="maintainability", severity="low",
    ...
    db.commit()
    db.add_all([
        models.SkillGroup(id=_id("skg-1"), name="后端规范组", description="Java 后端默认评估编组",
                          skill_ids=[_id("sk-1"), _id("sk-2")], analysis_type="repo_analysis", enabled=1,
                          created_at=now, updated_at=now),
        models.SkillGroup(id=_id("skg-2"), name="前端规范组", description="前端工程默认评估编组",
                          skill_ids=[_id("sk-3"), _id("sk-4")], analysis_type="repo_analysis", enabled=1,
                          created_at=now, updated_at=now),
    ])
    db.commit()
...
def _seed_capability(db: Session) -> None:
    ...
    for role_key, dimensions in ROLE_DIMENSIONS.items():
        role = models.CapabilityRole(
            id=_id(f"cr-{role_key}"), key=role_key, name=ROLE_NAMES[role_key],
            dimensions=list(dimensions), enabled=1, created_at=now, updated_at=now,
            skill_group_id=_id("skg-1") if role_key == "backend" else (_id("skg-2") if role_key == "frontend" else None),
        )
        db.add(role)
        for level in ALL_LEVELS:
            db.add(models.CapabilityStandard(
                id=_id(f"cstd-{role_key}-{level.lower()}"), role_id=role.id, level=level,
...
def _seed_env_inventory(db: Session) -> None:
    now = _now()
    scan_id = _id("einv-scan-1")
    db.add(models.EnvInventoryScan(
        id=scan_id, project_id=_id("p1"), scan_type="full", status="completed",
    ...
    for i, entry in enumerate(entries, start=1):
        db.add(models.EnvInventoryEntry(
            id=_id(f"einv-{i}"), project_id=_id("p1"), scan_id=scan_id, **entry,
...
def _seed_evaluations(db: Session) -> None:
    ...
    db.add_all([
        models.DeveloperEvaluation(
            id=_id("deval-1"), developer_id=_id("d1"), role_key="backend", skill_group_id=_id("skg-1"),
            project_id=_id("p1"), repo_path="/data/repos/user-center", git_author="陈思",
            ...
            rule_snapshot=snapshot("后端规范组", _id("skg-1"), [_id("sk-1"), _id("sk-2")], ["SQL 禁止字符串拼接", "金额使用 BigDecimal"]),
        ...
        models.DeveloperEvaluation(
            id=_id("deval-2"), developer_id=_id("d3"), role_key="frontend", skill_group_id=_id("skg-2"),
            project_id=_id("p3"), repo_path="/data/repos/content-engine", git_author="王琳",
            ...
            rule_snapshot=snapshot("前端规范组", _id("skg-2"), [_id("sk-3"), _id("sk-4")], ["禁止 console.log 提交", "禁止内联样式硬编码"]),
            achieved_level="E3", best_level="E3",
...
def _seed_portfolio_and_reports(db: Session) -> None:
    # 组合对比快照：每个项目几条历史趋势
    snapshots = [
        dict(pid=_id("p1"), score=84, quality=86, security=83, debt=26, recorded_at="2026-05-01T00:00:00+00:00"),
        dict(pid=_id("p1"), score=86, quality=88, security=84, debt=24, recorded_at="2026-06-01T00:00:00+00:00"),
        dict(pid=_id("p1"), score=88, quality=90, security=85, debt=22, recorded_at="2026-07-01T00:00:00+00:00"),
        dict(pid=_id("p2"), score=88, quality=90, security=87, debt=20, recorded_at="2026-05-01T00:00:00+00:00"),
        dict(pid=_id("p2"), score=89, quality=91, security=88, debt=19, recorded_at="2026-06-01T00:00:00+00:00"),
        dict(pid=_id("p2"), score=90, quality=92, security=88, debt=18, recorded_at="2026-07-01T00:00:00+00:00"),
        dict(pid=_id("p3"), score=90, quality=92, security=89, debt=17, recorded_at="2026-05-01T00:00:00+00:00"),
        dict(pid=_id("p3"), score=91, quality=93, security=90, debt=16, recorded_at="2026-06-01T00:00:00+00:00"),
        dict(pid=_id("p3"), score=92, quality=94, security=90, debt=15, recorded_at="2026-07-01T00:00:00+00:00"),
    ]
    ...
    for i, s in enumerate(snapshots, start=1):
        ...
        db.add(models.ProjectAssessmentSnapshot(
            id=_id(f"psnap-{i}"), project_id=s["pid"], score=s["score"], quality=s["quality"],
    ...
    # 报告导出记录
    db.add_all([
        models.ReportExport(id=_id("rpt-1"), report_type="project_comparison", format="html",
                            subject_ids=[_id("p1"), _id("p2"), _id("p3")], requested_by=DEFAULT_USER_ID,
                            created_at=_now()),
        models.ReportExport(id=_id("rpt-2"), report_type="developer_evaluation", format="pdf",
                            subject_ids=[_id("d1"), _id("deval-1")], requested_by=DEFAULT_USER_ID,
                            created_at=_now()),
    ])
    db.commit()
...
def _seed_tenant_and_membership(db: Session) -> None:
    ...
    if not user:
        ...
        db.flush()
    # 租户（幂等：已存在则跳过，避免主键冲突——tenant-default 等已存在租户）
    tenant = db.query(models.Tenant).filter_by(id=SEED_TENANT).first()
    if not tenant:
        db.add(models.Tenant(
            id=SEED_TENANT, name=TENANT_NAME, slug=TENANT_SLUG,
            status="active", created_at=now, updated_at=now,
        ))
    # 成员关系（幂等：该用户已是本租户成员则跳过）
    existing_mem = db.query(models.TenantMembership).filter_by(
        tenant_id=SEED_TENANT, user_id=DEFAULT_USER_ID).first()
    if not existing_mem:
        db.add(models.TenantMembership(
            id=_id("tmem-owner"), tenant_id=SEED_TENANT, user_id=DEFAULT_USER_ID,
...
def _seed_my_developer(db: Session) -> None:
    """tenant-default 专属：为 local-admin 账号创建关联的开发者档案。

    通过 Developer.username == AccountUser.username（_find_my_developer 优先级 1）
    关联，使个人中心「能力评估 / 参与项目 / 所在团队」模块均有数据。
    其它租户（如 tenant-qa）不受影响，保持原 6 开发者演示数据不变。
    """
    if SEED_TENANT != "tenant-default":
        return
    from .access import DEFAULT_USER_ID
    ...
    me_id = _id("dme")
    now = _now()
    display_name = user.name or "本地管理员"

    # 1) 开发者档案（与 local-admin 账号通过 username 关联）
    db.add(models.Developer(
        id=me_id,
        name=display_name,
        username=user.username,            # local-admin -> _find_my_developer 命中
        role="架构师",
        role_type="backend",
        team="后端小组",
        team_id=_id("a-backend"),           # TeamSpace id（FK -> team_spaces.id）
        level="E3",
        overall=86,
        commits=312, reviews=96,
        langs=["Go", "Python", "TypeScript"],
        tags=["核心模块贡献者", "全栈能力"],
        capability={"code_quality": 86, "architecture": 90, "stability": 83,
                    "efficiency": 80, "collaboration": 88, "security_aware": 78,
                    "test_coverage": 74, "growth_velocity": 79},
        ai_suggestion="作为本地管理员，你同时具备后端架构与全栈能力，建议持续主导核心模块并培养备份负责人。",
    ))

    # 2) 加入后端小组 / 测试团队（TeamSpace）成员
    for ts_id in [_id("a-backend"), _id("a-test-team")]:
        ts = db.query(models.TeamSpace).filter_by(id=ts_id, tenant_id=SEED_TENANT).first()
        if ts and me_id not in (ts.member_ids or []):
            ts.member_ids = list(ts.member_ids or []) + [me_id]
    # 同步聚合团队人数（后端小组 da-t2：原 2 人 + 我 = 3 人）
    t2 = db.query(models.Team).filter_by(id=_id("a-t2"), tenant_id=SEED_TENANT).first()
    if t2 and (t2.members or 0) < 3:
        t2.members = 3

    # 3) 最新一次能力评估（后端规范组，项目 用户中心）
    db.add(models.DeveloperEvaluation(
        id=_id("deval-me"), developer_id=me_id, role_key="backend",
        skill_group_id=_id("skg-1"), project_id=_id("p1"),
        repo_path="/data/repos/user-center", git_author=display_name,
        scores={"code_quality": 86, "architecture": 90, "stability": 83, "efficiency": 80,
                "collaboration": 88, "security_aware": 78, "test_coverage": 74, "growth_velocity": 79},
        evidence=[{"dimension": "architecture", "summary": "主导用户中心 auth-core 模块设计", "rules": ["架构评审通过"]}],
        rule_snapshot={
            "group_name": "后端规范组", "group_id": _id("skg-1"),
            "skill_ids": [_id("sk-1"), _id("sk-2")],
            "skills": [{"id": _id("sk-1"), "name": "SQL 禁止字符串拼接", "category": "security", "severity": "high",
                        "rule_content": "测试规则快照", "positive_examples": [], "negative_examples": []},
                       {"id": _id("sk-2"), "name": "金额使用 BigDecimal", "category": "security", "severity": "high",
                        "rule_content": "测试规则快照", "positive_examples": [], "negative_examples": []}],
        },
        achieved_level="E3", best_level="E3",
        gaps=[{"dimension": "security_aware", "current": 78, "target": 84, "gap": 6},
              {"dimension": "test_coverage", "current": 74, "target": 80, "gap": 6}],
        summary="后端架构师，架构能力突出（90），达到 E3 标准；安全意识与测试覆盖可进一步提升。",
        status="completed", error="", created_at=now, updated_at=now,
    ))

    # 4) 在 用户中心 / 订单系统 的 contributor_list 登记，使「参与项目」模块有数据
    me_contrib = {"name": display_name, "username": user.username,
                  "commits": 142, "reviews": 48, "ownership": 58}
    for pid in [_id("p1"), _id("p2")]:
        p = db.query(models.Project).filter_by(id=pid, tenant_id=SEED_TENANT).first()
        if p:
            cl = list(p.contributor_list or [])
            if not any(c.get("username") == user.username for c in cl):
                cl.append(me_contrib)
                p.contributor_list = cl
...
def _seed_all() -> None:
    ...
    _seed_my_developer(db)
    db.close()


def seed_test_tenant() -> None:
    """tenant-qa：独立可删重建的演示租户（ID 前缀 q）。"""
    configure("tenant-qa", "QA 测试组织", "qa-test", "q", preserve_tenant=False)
    _seed_all()
    print("✓ tenant-qa seed 完成")


def seed_default_tenant() -> None:
    """tenant-default：本地管理员默认工作区（ID 前缀 d，保留租户行与成员关系）。"""
    configure("tenant-default", "DevLens 本地工作区", "local", "d", preserve_tenant=True)
    _seed_all()
    print("✓ tenant-default seed 完成")
