...
| 文件 | 说明 |
|---|---|
| `deploy/nginx-devlens.conf` | TLS 反代配置：listen 7504 ssl；`/api/` → 127.0.0.1:8000（含 `proxy_set_header` 透传 X-DevLens-* 与 Host）；其余 → 127.0.0.1:3800 |
...
- Let's Encrypt 正式证书（自定义端口 7504 + 内网主机名不适用）。
