...
import { developers, projects, repoList } from '@/lib/mock-data';
import type { TeamSpace } from '@/lib/types';

function SpaceSheet({ space, open, onClose }: { space: TeamSpace | null; open: boolean; onClose: () => void }) {
  ...
  const spaceRepos = repoList.filter((repo) => repo.teamId === space.id);
  const addGroup = (event: React.FormEvent) => {
    event.preventDefault();
    if (!groupName.trim()) return;
    createTeamGroup({ teamId: space.id, name: groupName.trim() });
    setGroupName('');
  };
...
function CreateSpaceSheet({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const submit = (event: React.FormEvent) => {
    ...
    createTeamSpace({ name: name.trim(), description: description.trim(), largeTeamId, ownerId: owner?.id, ownerName: owner?.name });
...
export default function TeamSpacesPage() {
  ...
  const unassignedRepos = repoList.filter((repo) => !repo.projectId).length;
  ...
  return <><PageHeader title="团队空间管理" description="团队是项目、人员与小组的组织根；接入项目之前需要先归属团队。" actions={<Button variant="accent" onClick={() => setCreateOpen(true)}><Plus className="h-4 w-4" />新建团队空间</Button>} />
    ...
    <CreateSpaceSheet open={createOpen} onClose={() => setCreateOpen(false)} /><SpaceSheet space={selected} open={Boolean(selected)} onClose={() => setSelected(null)} />
  </>;
}
