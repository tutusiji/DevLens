...
import { Waypoints, FileCode2, GitFork, Boxes } from 'lucide-react';
import { Card, CardContent, CardHeader, CardTitle, CardDescription } from '@/components/ui/card';
import { Badge } from '@/components/ui/badge';
import { PageHeader } from '@/components/widgets';
import { GraphCanvas, type GraphNode } from '@/components/graph-canvas';
import { graphModules, graphEdges, layerColors } from '@/lib/mock-data';
...
const graphNodes: GraphNode[] = graphModules.map((m) => ({
  id: m.id,
  x: m.x,
  y: m.y,
  size: 20,
  color: layerColors[m.layer],
  label: m.label,
  sublabel: `${m.loc} · ${m.health}`,
}));

const graphStats = [
  { label: '模块总数', value: graphModules.length, icon: Boxes },
  { label: '依赖关系', value: graphEdges.length, icon: GitFork },
  { label: '代码总量', value: '52k', unit: 'LOC', icon: FileCode2 },
  { label: '平均健康度', value: 84, unit: '分', icon: Waypoints },
];
  ...
  return (
    <>
      <PageHeader
        ...
        actions={
          <Badge variant="outline" className="font-mono">payment-core</Badge>
        }
      />
        ...
        <Card className="lg:col-span-3">
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                ...
                <CardDescription>悬停节点高亮直接依赖 · 节点标注代码量与健康度</CardDescription>
              </div>
          ...
          <CardContent>
            <div className="rounded-xl bg-[radial-gradient(circle_at_center,var(--secondary)_0%,transparent_70%)]">
              <GraphCanvas nodes={graphNodes} links={graphEdges} height={480} />
            </div>
          </CardContent>
        </Card>
              ...
              .map((m) => (
                ...
                >
                  <span
                    ...
                    style={{ backgroundColor: layerColors[m.layer] }}
                  />
                  <div className="min-w-0 flex-1">
                    <p className="truncate font-mono text-sm">{m.label}</p>
                    <p className="text-[11px] text-muted-foreground">
                      {layerLabels[m.layer]} · {m.loc} LOC
                    </p>
                  </div>
                  <span
